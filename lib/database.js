...
Database.prototype.initialize = function(callback) {
  ...
  function finishInitializing(err, db) {
    ...
    // Block hashes -> detached/orphaned blocks ( blocks that were deleted during evm_revert )
    self.blockHashesOrphaned = new LevelUpObjectAdapter("blockHashesOrphaned", self.db, blockserializer);
