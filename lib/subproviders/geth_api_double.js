 ...
 * evm_mine
 *
 * Mine a new block. Optionally, mine it at a desired timestamp. Optionally, do
 * not emit the event that a new block was mined ( by default, the event is
 * emitted ).
 *
 * @param  {Object} params Object that may contain `timestamp` and
 * `emitNewHeaderEvent` fields ( optional ).
 * @param  {Function} callback Callback function when transaction processing is completed.
 * @return [type]              [description]
 */
GethApiDouble.prototype.evm_mine = function(params, callback) {
  if (typeof params === "function") {
    callback = params;
    params = {};
  }
  this.state.processBlock(params, function(err) {
    if (err) {
      return callback(err);
    }
    callback(err, "0x0");
