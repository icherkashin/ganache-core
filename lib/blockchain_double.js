...
BlockchainDouble.prototype.getBlock = function(number, callback) {
  ...
  if (number.indexOf("0x") >= 0) {
    ...
    // block hash
    if (hash.length > 40) {
      this.data.blockHashes.get(to.hex(hash), (err, blockIndex) => {
        if (err) {
          // If the block ID was not found by the associated block hash, the
          // block might have been orphaned during evm_revert.
          // If the block with a given block hash never existed in the chain,
          // then it could never be orphaned, so an attempt to retrieve a block
          // given an incorrect hash will properly fail.
          return this.data.blockHashesOrphaned.get(to.hex(hash), callback);
        }
        return this.data.blocks.get(blockIndex, callback);
      });
    } else {
      // Block number
...
BlockchainDouble.prototype.popBlock = function(callback) {
  ...
  this.data.blocks.last(function(err, block) {
    ...
    requests.push(
      // Save the orphaned block's information, in the interest of simulating chain reorganization
      self.data.blockHashesOrphaned.set.bind(self.data.blockHashesOrphaned, to.hex(block.hash()), block),
