          ...
          async.eachSeries(
            ...
            function(err) {
              ...
              self.vm.stateManager.commit(() => {
                // Create first block
                self.putBlock(block, [], [], true, callback);
              });
            }
          );
        });
...
BlockchainDouble.prototype.putBlock = function(block, logs, receipts, emitNewHeaderEvent, callback) {
  var self = this;
 ...
 */
BlockchainDouble.prototype.processBlock = function(vm, block, commit, emitNewHeaderEvent, callback) {
  var self = this;

  if (typeof emitNewHeaderEvent === "function") {
    callback = emitNewHeaderEvent;
    commit = true;
    emitNewHeaderEvent = true;
  }
  ...
  vm.runBlock(
    ...
    async function(vmerr, results) {
      ...
      if (commit) {
        ...
        self.putBlock(block, logs, receipts, emitNewHeaderEvent, done);
      } else {
        done();
      }
 ...
 *
 * @param  {Object} params Object that may provide the following fields:
 * `timestamp` at which the block is mined, and `emitNewHeaderEvent`, which is
 * true by default.
 * @param  {Function} callback Callback when transaction processing is finished.
 * @return [type]              [description]
 */
BlockchainDouble.prototype.processNextBlock = function(params, callback) {
  var self = this;
  ...
  const { timestamp = null, emitNewHeaderEvent = true } = params;
  ...
  this.createBlock(function(err, block) {
    ...
    self.processBlock(self.vm, block, true, emitNewHeaderEvent, callback);
  });
};
  ...
  this.getTransactionReceipt(targetHash, function(err, receipt) {
    ...
    self.getBlock(targetBlock.header.parentHash, function(err, parent) {
      ...
      self.createBlock(parent, function(err, block) {
        ...
        self.processBlock(vm, block, false, true, function(err) {
